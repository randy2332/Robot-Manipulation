...
from fk import your_fk, get_ur5_DH_params ,calculate_jacobian
...
def your_ik(robot_id, new_pose : list or tuple or np.ndarray, 
                base_pos, max_iters : int=1500, stop_thresh : float=.001):
    ...
    DH_params = get_ur5_DH_params () #get DH params from fk file
    base_pose = list(base_pos) + [0, 0, 0 ,1] # 7 element
    #print("base_pose",base_pose)
    A = get_matrix_from_pose(base_pose) 
    
    alpha = 1
    x_cur, _ = your_fk(DH_params, tmp_q, base_pos) # we have to know current x it is 7dof
 
    delta_x = np.array(pose_7d_to_6d(new_pose)) - np.array(pose_7d_to_6d(x_cur)) # 6d
    ...
    while cur_it < max_iters:
        J = calculate_jacobian(DH_params , q_new )
        J_three = J.T @ np.linalg.inv(J @ J.T)
        J_sharp = np.multiply(alpha,J_three).astype(float) 
        delta_theta = np.dot(J_sharp , delta_x).astype(float) 
        #print("delta_theta ",delta_theta )
        q_new = q_new + delta_theta 
        q_new = np.squeeze(np.array(q_new)) 
        # if not is_within_limits(joint_limits, q_new):  
        #     print("[Info] Joint limits reached! Aborting and correct")
        #     break

        tmp_q = q_new
        x_cur, _ = your_fk( DH_params, tmp_q,base_pos)
