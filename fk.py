...
def calculate_transform(DH_params,num ,q):
    ...
    for i, params in enumerate(DH_params):
        ...
        A = A@transform
        #need this when compute jacobian
        if i == num:
            break
...
def calculate_jacobian(DH_params , q ):
    num_joints = 6
    jacobian = np.zeros((6, num_joints))

    for i in range(num_joints):#0 to 5
        '''
        calculate_transform(i,q)[:3,:3] is to get R 
        and we also need to include base_pose as we did in compute A 
        '''
        #R0 should be indendity  
        if i == 0:
            down_element = np.matrix([[0],
                                      [0],
                                      [1]])
        else : 
            down_element = calculate_transform(DH_params,i-1,q)[:3,:3] @ np.matrix([[0],
                                                                                    [0],
                                                                                    [1]])
        #print("down_element",down_element)
        d= di(DH_params,q,i)
        if i == 0:
            up_element = np.cross(np.squeeze(np.matrix([[0],[0],[1]])), np.squeeze(d))
        else:
            up_element = np.cross(np.squeeze(down_element), np.squeeze(d))
        
        jacobian[:, i] = np.hstack((up_element ,down_element.T))
    return jacobian



def di(DH_params,q,i):
    d_0_6 =   calculate_transform(DH_params,5,q)@ np.matrix([[0],
                                                             [0],
                                                             [0],
                                                             [1]])
    if i == 0:
        ...
        d_0_i = calculate_transform(DH_params,i-1,q) @ np.matrix([[0],
                                                                  [0],
                                                                  [0],
                                                                  [1]])
...
def your_fk(DH_params : dict, q : list or tuple or np.ndarray, base_pos) -> np.ndarray:
    ...
    A = A@calculate_transform(DH_params,5,q)    
    jacobian  = calculate_jacobian(DH_params , q )
